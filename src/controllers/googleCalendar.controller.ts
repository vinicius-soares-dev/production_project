...
export const syncEmployeeCalendar = async (req: Request, res: Response) => {
  try {
...
// Funções auxiliares
const createEventsFromSchedule = (schedule: Record<string, string[]>) => {
  return Object.entries(schedule).flatMap(([day, times]) => 
    times.map(time => ({
...
const createEventTime = (time: string) => ({
  dateTime: `2023-01-01T${time}:00-03:00`,
  timeZone: 'America/Sao_Paulo'
});

const mapDayToGoogle = (day: string) => ({
  seg: 'MO',
  ter: 'TU',
  qua: 'WE',
  qui: 'TH',
...
const mapDayToColor = (day: string) => ({
  seg: '1',   // Azul
  ter: '2',   // Verde
  qua: '3',   // Roxo
  qui: '4',   // Vermelho
...
const addEventsToCalendar = async (auth: any, calendarId: string, events: any[]) => {
  return Promise.all(
    events.map(event => 
