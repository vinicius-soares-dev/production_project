        ...
        const ordersWithDetails = await Promise.all(orders.map(async (order) => {
          ...
          WHERE od.os_id = ?`, [order.id]);
            ...
            const departmentsWithCollaborators = await Promise.all(departments.map(async (dept) => {
                const [collaborators] = await pool.query(`SELECT 
                collaborator_id 
    ...
    try {
        ...
        try {
            ...
            if (Object.keys(updateData).length > 0) {
                await connection.query('UPDATE service_orders SET ? WHERE id = ?', [updateData, id]);
            }
            // 2. Atualiza departamentos
            if (departments) {
                await connection.query('DELETE FROM os_departments WHERE os_id = ?', [id]);
                if (departments.length > 0) {
                    await connection.query('INSERT INTO os_departments (os_id, department_id) VALUES ?', [departments.map((d) => [id, d])]);
                }
            }
