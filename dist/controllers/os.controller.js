...
export const createServiceOrder = async (req, res, next) => {
    const { os_number, departments, service_days } = req.body;
    ...
    try {
        ...
        try {
            ...
            const [existingOS] = await connection.query('SELECT id FROM service_orders WHERE os_number = ?', [os_number]);
            if (existingOS.length > 0) {
                await connection.rollback();
                return res.status(409).json({ message: 'Número de OS já existe' });
            }
            // Inserir OS principal
            const [osResult] = await connection.query('INSERT INTO service_orders (os_number) VALUES (?)', [os_number]);
            const osId = osResult.insertId;
            // Inserir dias de serviço
            await connection.query('INSERT INTO os_service_days (os_id, day_of_week) VALUES ?', [uniqueDays.map(day => [osId, day])]);
            ...
            for (const dept of departments) {
                ...
                const [deptResult] = await connection.query(`INSERT INTO os_departments 
          (os_id, department_id, execution_start, execution_end) 
          VALUES (?, ?, ?, ?)`, [osId, dept.department_id, dept.execution_start, dept.execution_end]);
                // Inserir colaboradores
                if (dept.collaborator_ids?.length > 0) {
                    // Validar colaboradores
                    const [existingCollabs] = await connection.query('SELECT id FROM employees WHERE id IN (?)', [dept.collaborator_ids]);
                    if (existingCollabs.length !== dept.collaborator_ids.length) {
                        await connection.rollback();
                        return res.status(400).json({ message: 'Colaborador(es) inválido(s)' });
                    }
                    await connection.query('INSERT INTO os_collaborators (os_department_id, collaborator_id) VALUES ?', [dept.collaborator_ids.map(cId => [deptResult.insertId, cId])]);
                }
            }
...
const getOSDetails = async (osId) => {
    const connection = await pool.getConnection();
    try {
        const [osData] = await connection.query('SELECT * FROM service_orders WHERE id = ?', [osId]);
        const [departments] = await connection.query(`SELECT * FROM os_departments WHERE os_id = ?`, [osId]);
        const [collaborators] = await connection.query(`SELECT * FROM os_collaborators WHERE os_department_id IN (?)`, [departments.map(d => d.id)]);
        // Buscar dias de serviço
        const [serviceDays] = await connection.query('SELECT day_of_week FROM os_service_days WHERE os_id = ?', [osId]);
        return {
            ...osData[0],
...
export const deleteServiceOrder = async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            const [os] = await connection.query('SELECT id FROM service_orders WHERE id = ?', [osId]);
            if (os.length === 0) {
                await connection.rollback();
                return res.status(404).json({ message: 'Ordem de serviço não encontrada' });
            }
            // Obter IDs dos departamentos relacionados
            const [departments] = await connection.query('SELECT id FROM os_departments WHERE os_id = ?', [osId]);
            const departmentIds = departments.map(d => d.id);
            // Deletar colaboradores associados
            if (departmentIds.length > 0) {
                await connection.query('DELETE FROM os_collaborators WHERE os_department_id IN (?)', [departmentIds]);
            }
            // Deletar departamentos
            await connection.query('DELETE FROM os_departments WHERE os_id = ?', [osId]);
            // Deletar dias de serviço
            await connection.query('DELETE FROM os_service_days WHERE os_id = ?', [osId]);
            // Deletar OS principal
            await connection.query('DELETE FROM service_orders WHERE id = ?', [osId]);
            await connection.commit();
...
export const getServiceOrderById = async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            const departmentsWithCollaborators = await Promise.all(departments.map(async (dept) => {
                const [collaborators] = await connection.query(`SELECT collaborator_id FROM os_collaborators 
            WHERE os_department_id = ?`, [dept.id]);
                return {
                    ...dept,
